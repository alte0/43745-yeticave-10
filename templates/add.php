  ...
  <form class="form form--add-lot container <?= !empty($errors) ? "form--invalid" : "" ?>" action="add.php" method="post" enctype="multipart/form-data">
    ...
    <div class="form__container-two">
      ...
      <div class="form__item <?= isset($errors["category"]) ? "form__item--invalid" : "" ?>">
        ...
        <span class="form__error">Выберите категорию<?= isset($errors["category"]) ? addCommaAndSpaceText($errors["category"]) : "" ?></span>
      </div>
    </div>
    <div class="form__item form__item--wide <?= isset($errors["message"]) ? "form__item--invalid" : "" ?>">
      <label for="message">Описание <sup>*</sup></label>
      <textarea id="message" name="message" placeholder="Напишите описание лота"><?= getPostVal("message") ?></textarea>
      <span class="form__error">Напишите описание лота<?= isset($errors["message"]) ? addCommaAndSpaceText($errors["message"]) : "" ?></span>
    </div>
    ...
    <div class="form__container-three">
      <div class="form__item form__item--small <?= isset($errors["lot-rate"]) ? "form__item--invalid" : "" ?>">
        ...
        <span class="form__error">Введите начальную цену<?= isset($errors["lot-rate"]) ? addCommaAndSpaceText($errors["lot-rate"]) : "" ?></span>
      </div>
      <div class="form__item form__item--small <?= isset($errors["lot-step"]) ? "form__item--invalid" : "" ?>">
        <label for="lot-step">Шаг ставки <sup>*</sup></label>
        <input id="lot-step" type="text" name="lot-step" placeholder="0" value="<?= getPostVal("lot-step") ?>">
        <span class="form__error">Введите шаг ставки<?= isset($errors["lot-step"]) ? addCommaAndSpaceText($errors["lot-step"]) : "" ?></span>
      </div>
